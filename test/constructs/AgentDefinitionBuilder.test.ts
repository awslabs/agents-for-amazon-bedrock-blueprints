...
import { AgentDefinitionBuilder, PromptType, PromptConfig_Override, PromptConfig_Default, PromptStateConfig } from '../../constructs/AgentDefinitionBuilder';
import { USER_INPUT_ACTION_NAME, USER_INPUT_PARENT_SIGNATURE } from '../../constructs/src/utilities/constants';
...
describe('AgentDefinitionBuilder', () => {
    ...
    it('should set defaults correctly', () => {
        builder.withAgentName(agentName)
    ...
    it('should throw an error if override lambda is not provided for OVERRIDDEN parser mode', () => {
        ...
        expect(() => builder.build()).toThrowError('`overrideLambda` field must be set in `promptOverrideConfigurations` if `parserMode` is set to `OVERRIDDEN`');
    });
    ...
    it('should throw an error if same prompt type is redefined', () => {
        ...
        builder.withAgentName(agentName)
            .withInstruction(instruction)
            .withPreprocessingPrompt(prompt);
        expect(() => builder.withPreprocessingPrompt(prompt)).toThrowError('PRE_PROCESSING is already defined for this agent definition');
    });
