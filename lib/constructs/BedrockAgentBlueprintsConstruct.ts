...
export class BedrockAgentBlueprintsConstruct extends Construct {
  ...
  private setupIAMRole(): Role {
    ...
    bedrockServiceRole.addToPolicy(
      new PolicyStatement({
        ...
        resources: [`arn:aws:bedrock:${this.region}::foundation-model/${modelUsed}`],
      })
    );
  ...
  private associateActionGroupInfo(actionGroups: AgentActionGroup[] | undefined) {
    ...
    this.agentDefinition = {
      ...this.agentDefinition,
      actionGroups: [...existingActions, ...agentActionGroups],
    };
  }
   ...
   */
  private uploadSchemaToS3(
    schemaFile: Buffer | undefined,
    actionGroupName: string
  ): bedrock.CfnAgent.S3IdentifierProperty {
