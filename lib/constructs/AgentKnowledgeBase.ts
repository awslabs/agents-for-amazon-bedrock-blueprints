...
export class AgentKnowledgeBase extends Construct {
  ...
  constructor(scope: Construct, id: string, props: AgentKnowledgeBaseProps) {
    ...
    const storageConfig =
      props.storageConfiguration?.type ?? KnowledgeBaseStorageConfigurationTypes.OPENSEARCH_SERVERLESS; // Default to OpenSearchServerless
  ...
  public addAssetFiles(files: Buffer[] | FileBufferMap | undefined) {
    ...
    const fileBufferMap: FileBufferMap = Array.isArray(files) ? generateFileBufferMap(files) : files;
        ...
        DataSyncAccess: new PolicyDocument({
          statements: [
            new PolicyStatement({
              ...
              ], // Update a data source associated with the knowledgebase
  ...
  private setupOpensearchServerless(kbName: string, region: string, accountId: string) {
    ...
    const permissionCustomResource = this.waitForPermissionPropagation(
      validationLambdaExecutionRole,
      aossCollection.collection.attrCollectionEndpoint,
      aossCollection.indexName
    );
